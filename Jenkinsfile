    ...
    stage('Create kube config file') {
      steps{
        withAWS(credentials: 'awscredentials',region: 'us-west-2') {
          sh 'aws eks update-kubeconfig --name abcapstone5'
        }
      }
    }
    stage('Deploy blue container'){
      steps {
        withKubeConfig([credentialsId: 'kubectlid']) {
          ...
          sh 'kubectl config use-context arn:aws:eks:us-west-2:406401063468:cluster/abcapstone5 --kubeconfig=/var/jenkins_home/.kube/config'
